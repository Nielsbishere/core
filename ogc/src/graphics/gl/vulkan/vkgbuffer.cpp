...
bool GBuffer::init() {
	...
	//info.changes.resize(VkGBuffer::isVersioned(info.type) ? g->getBuffering() : 1); TODO:
	info.changes.resize(1);
...
void GBuffer::push() {
	...
	Vec2u &changes = info.changes[g->getExtension().current % (u32) info.changes.size()];
	...
	//Map memory
	u8 *addr;
	vkCheck<0x2, GBuffer>(vkMapMemory(g->getExtension().device, ext.memory, off, len, 0, (void**)&addr), "Couldn't map memory");
