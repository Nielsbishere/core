...
Grad::Grad(flp x, flp y, flp z, flp w) : x(x), y(y), z(z), w(w) { }

Grad::Grad(flp x, flp y, flp z) : Grad(x, y, z, (flp) 0) {}
...
flp SimplexNoise::F2 = (flp) 0.5 * (sqrt((flp) 3.0) - (flp) 1.0);
flp SimplexNoise::G2 = ((flp) 3.0 - sqrt((flp) 3.0)) / (flp) 6.0;
flp SimplexNoise::F4 = (sqrt((flp) 5.0) - (flp) 1.0) / (flp) 4.0;
flp SimplexNoise::G4 = ((flp) 5.0 - sqrt((flp) 5.0)) / (flp) 20.0;
flp SimplexNoise::F3 = (flp) 1.0 / (flp) 3.0;
flp SimplexNoise::G3 = (flp) 1.0 / (flp) 6.0;

// 2D simplex noise
flp SimplexNoise::noise(Vec2 p) {
	flp xin = p.x;
	flp yin = p.y;

	flp n0, n1, n2; // Noise contributions from the three corners
					// Skew the input space to determine which simplex cell we're in
	flp s = (xin + yin) * F2; // Hairy factor for 2D
	i32 i = fastfloor(xin + s);
	i32 j = fastfloor(yin + s);
	flp t = (i + j) * G2;
	flp X0 = i - t; // Unskew the cell origin back to (x,y) space
	flp Y0 = j - t;
	flp x0 = xin - X0; // The x,y distances from the cell origin
	flp y0 = yin - Y0;
	...
	if (x0 > y0) {
		...
		j1 = 1;
		   ...
		   // c = (3-sqrt(3))/6
	flp x1 = x0 - i1 + G2; // Offsets for middle corner in (x,y) unskewed coords
	flp y1 = y0 - j1 + G2;
	flp x2 = x0 - (flp) 1.0 + (flp) 2.0 * G2; // Offsets for last corner in (x,y) unskewed coords
	flp y2 = y0 - (flp) 1.0 + (flp) 2.0 * G2;
	...
	return ((flp) 70.0 * (n0 + n1 + n2) + 1) / 2;
}

// 3D simplex noise
flp SimplexNoise::noise(Vec3 p) {
	flp xin = p.x;
	flp yin = p.y;
	flp zin = p.z;

	flp n0, n1, n2, n3; // Noise contributions from the four corners
						// Skew the input space to determine which simplex cell we're in
	flp s = (xin + yin + zin) * F3; // Very nice and simple skew factor for 3D
	i32 i = fastfloor(xin + s);
	i32 j = fastfloor(yin + s);
	i32 k = fastfloor(zin + s);
	flp t = (i + j + k) * G3;
	flp X0 = i - t; // Unskew the cell origin back to (x,y,z) space
	flp Y0 = j - t;
	flp Z0 = k - t;
	flp x0 = xin - X0; // The x,y,z distances from the cell origin
	flp y0 = yin - Y0;
	flp z0 = zin - Z0;
	...
	// c = 1/6.
	flp x1 = x0 - i1 + G3; // Offsets for second corner in (x,y,z) coords
	flp y1 = y0 - j1 + G3;
	flp z1 = z0 - k1 + G3;
	flp x2 = x0 - i2 + (flp) 2.0 * G3; // Offsets for third corner in (x,y,z) coords
	flp y2 = y0 - j2 + (flp) 2.0 * G3;
	flp z2 = z0 - k2 + (flp) 2.0 * G3;
	flp x3 = x0 - (flp) 1.0 + (flp) 3.0 * G3; // Offsets for last corner in (x,y,z) coords
	flp y3 = y0 - (flp) 1.0 + (flp) 3.0 * G3;
	flp z3 = z0 - (flp) 1.0 + (flp) 3.0 * G3;
	...
	flp t0 = (flp) 0.6 - x0 * x0 - y0 * y0 - z0 * z0;
	if (t0 < 0)
		n0 = (flp) 0.0;
	else {
		t0 *= t0;
	...
	flp t1 = (flp) 0.6 - x1 * x1 - y1 * y1 - z1 * z1;
	if (t1 < 0)
		n1 = (flp) 0.0;
	else {
		t1 *= t1;
	...
	flp t2 = (flp) 0.6 - x2 * x2 - y2 * y2 - z2 * z2;
	if (t2 < 0)
		n2 = (flp) 0.0;
	else {
		t2 *= t2;
	...
	flp t3 = (flp) 0.6 - x3 * x3 - y3 * y3 - z3 * z3;
	if (t3 < 0)
		n3 = (flp) 0.0;
	else {
		t3 *= t3;
	...
	return (((flp) 32.0 * (n0 + n1 + n2 + n3)) + 1) / 2;
}

// 4D simplex noise, better simplex rank ordering method 2012-03-09
flp SimplexNoise::noise(Vec4 p) {
	flp x = p.x;
	flp y = p.y;
	flp z = p.z;
	flp w = p.w;

	flp n0, n1, n2, n3, n4; // Noise contributions from the five corners
							// Skew the (x,y,z,w) space to determine which cell of 24 simplices we're in
	flp s = (x + y + z + w) * F4; // Factor for 4D skewing
	i32 i = fastfloor(x + s);
	i32 j = fastfloor(y + s);
	i32 k = fastfloor(z + s);
	i32 l = fastfloor(w + s);
	flp t = (i + j + k + l) * G4; // Factor for 4D unskewing
	flp X0 = i - t; // Unskew the cell origin back to (x,y,z,w) space
	flp Y0 = j - t;
	flp Z0 = k - t;
	flp W0 = l - t;
	flp x0 = x - X0;  // The x,y,z,w distances from the cell origin
	flp y0 = y - Y0;
	flp z0 = z - Z0;
	flp w0 = w - W0;
	...
	flp x1 = x0 - i1 + G4; // Offsets for second corner in (x,y,z,w) coords
	flp y1 = y0 - j1 + G4;
	flp z1 = z0 - k1 + G4;
	flp w1 = w0 - l1 + G4;
	flp x2 = x0 - i2 + (flp) 2.0 * G4; // Offsets for third corner in (x,y,z,w) coords
	flp y2 = y0 - j2 + (flp) 2.0 * G4;
	flp z2 = z0 - k2 + (flp) 2.0 * G4;
	flp w2 = w0 - l2 + (flp) 2.0 * G4;
	flp x3 = x0 - i3 + (flp) 3.0 * G4; // Offsets for fourth corner in (x,y,z,w) coords
	flp y3 = y0 - j3 + (flp) 3.0 * G4;
	flp z3 = z0 - k3 + (flp) 3.0 * G4;
	flp w3 = w0 - l3 + (flp) 3.0 * G4;
	flp x4 = x0 - (flp) 1.0 + (flp) 4.0 * G4; // Offsets for last corner in (x,y,z,w) coords
	flp y4 = y0 - (flp) 1.0 + (flp) 4.0 * G4;
	flp z4 = z0 - (flp) 1.0 + (flp) 4.0 * G4;
	flp w4 = w0 - (flp) 1.0 + (flp) 4.0 * G4;
	...
	flp t0 = (flp) 0.6 - x0 * x0 - y0 * y0 - z0 * z0 - w0 * w0;
	if (t0 < 0)
		n0 = (flp) 0.0;
	else {
		t0 *= t0;
	...
	flp t1 = (flp) 0.6 - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1;
	if (t1 < 0)
		n1 = (flp) 0.0;
	else {
		t1 *= t1;
	...
	flp t2 = (flp) 0.6 - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2;
	if (t2 < 0)
		n2 = (flp) 0.0;
	else {
		t2 *= t2;
	...
	flp t3 = (flp) 0.6 - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3;
	if (t3 < 0)
		n3 = (flp) 0.0;
	else {
		t3 *= t3;
	...
	flp t4 = (flp) 0.6 - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4;
	if (t4 < 0)
		n4 = (flp) 0.0;
	else {
		t4 *= t4;
	...
	return ((flp) 27.0 * (n0 + n1 + n2 + n3 + n4) + 1) / 2;
}

flp SimplexNoise::noise(flp p) {
	return noise(Vec2((flp) p, (flp) 0.0));
}

flp SimplexNoise::noise(flp p, i32 octaves, flp persistance, flp lacunarity) {
	flp freq = 1, amp = 1;
	flp y = 0;
	flp max = 0;
...
flp SimplexNoise::noise(Vec2 p, i32 octaves, flp persistance, flp lacunarity) {
	flp freq = 1, amp = 1;
	flp z = 0;
	flp max = 0;
	for (i32 i = 0; i < octaves; i++) {
		z += (noise(p * freq) + 1) / 2 * amp;
		max += amp;
		amp *= persistance;
		freq *= lacunarity;
	}
...
flp SimplexNoise::noise(Vec3 p, i32 octaves, flp persistance, flp lacunarity) {
	flp freq = 1, amp = 1;
	flp w = 0;
	flp max = 0;
	for (i32 i = 0; i < octaves; i++) {
		w += (noise(p * freq) + 1) / 2 * amp;
		max += amp;
		amp *= persistance;
		freq *= lacunarity;
	}
...
flp SimplexNoise::noise(Vec4 p, i32 octaves, flp persistance, flp lacunarity) {
	flp freq = 1, amp = 1;
	flp w = 0;
	for (i32 i = 0; i < octaves; i++) {
		w += (noise(p * freq) + 1) / 2 * amp;
...
i32 SimplexNoise::fastfloor(flp x) {
	i32 xi = (i32)x;
	return x < xi ? xi - 1 : xi;
}

flp SimplexNoise::dot(Grad g, flp x, flp y) {
	return g.x * x + g.y * y;
}

flp SimplexNoise::dot(Grad g, flp x, flp y, flp z) {
	return g.x * x + g.y * y + g.z * z;
}

flp SimplexNoise::dot(Grad g, flp x, flp y, flp z, flp w) {
