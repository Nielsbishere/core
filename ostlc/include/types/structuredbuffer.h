...
namespace oi {
	...
	public:
		...
		StructuredBufferVar();
		StructuredBufferVar(StructuredBuffer *buf, u32 id, String name, DataType type, u32 offset, u32 stride, u32 length, u32 parent);
		...
		std::vector<u32> getDimensions() const;
	...
	public:
		...
		StructuredBufferVar &find(String s);			//By name

		u32 size() const;
		u32 dataSize() const;

		bool contains(u32 i) const;						//Contains id
		bool contains(String s) const;					//Contains name
		u32 find(StructuredBufferVar *var) const;		//Get index from value

		Buffer getBuffer();
		void setBuffer(Buffer buf);

		static String simplifyPath(String str);		//Gets rid of brackets

		//Add SBV (by fully qualified name)
		//Stride = 0; default stride (length of data type)
		StructuredBufferVar &add(String name, DataType type, u32 offset, u32 stride = 0, u32 arrayLength = 1, StructuredBufferVar *parent = nullptr);
		...
		bool addAll(String path, DataType type, u32 offset, u32 stride = 0, u32 arrayLength = 1);

		BufferVar operator[](String path);	//By path

	private:

		Buffer buf;
		std::unordered_map<String, StructuredBufferVar> vars;
	};
