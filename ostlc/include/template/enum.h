...
//TODO: SEnum is undebuggable, make it a template class instead
...
structInfo																						\
																								\
name##_s operator=(const name##_s &other) { memcpy(this, &other, sizeof(*this)); return *this; }\
constexpr name##_s operator=(const name##_s&) const { return *this; }							\
};																								\
																								\
class name {																					\
																								\
...
private:																						\
																								\
	static const constexpr name##_s ilist[] = { a0, __VA_ARGS__ };								\
																								\
public:																							\
																								\
	...
	const oi::String getName() const { return getNames()[index]; }								\
																								\
	static oi::Array<name##_s> getValues() {													\
		return oi::Array<name##_s>(ilist, ilist + length);										\
	}																							\
																								\
...
protected:																						\
	...
	static const std::vector<oi::String> initNames() {											\
		...
		oi::Array<oi::String> arr = getArgs().split("=");										\
																								\
		u32 i = 0;																				\
		for (oi::String &temp : arr) {															\
																								\
			if (i == 0){																		\
				oi::Array<oi::String> csplit2 = temp.split("(");								\
				res.push_back(csplit2[csplit2.size() - 1]);										\
			}																					\
			else if (i < arr.size() - 1) {														\
				oi::Array<oi::String> csplit = temp.split(",");									\
				oi::String almostCorrect = csplit[csplit.size() - 1];							\
				oi::Array<oi::String> csplit2 = almostCorrect.split("(");						\
				res.push_back(csplit2[csplit2.size() - 1]);										\
			}																					\
																								\
			++i;																				\
